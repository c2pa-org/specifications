...
A _manifest consumer_ whose role is to perform the actions described in [Chapter 16, _Validation_](#_validation).
...
Some assertion data, due to its size or an infrequent need for it, may be externally hosted. Such data are not embedded in the assertion store, but instead are referenced by URI. This is accomplished through a cloud data assertion (see [Section 19.10, “Cloud Data”](#_cloud_data)). Unlike embedded assertion data, cloud data is not retrieved nor validated as part of manifest validation, and are only retrieved and validated when specifically needed by an application according to a different set of validation rules as described in [Section 16.7, “Validate the Assertions”](#_validate_the_assertions).
...
> **NOTE:**
> More details on hashing can be found at [Section 14.1, “Hashing”](#_hashing).
...
Some assertions, such as [Creative Work](#_creative_work) and [Actions](#_actions), may contain references to Persons or Organisations which are responsible for various roles and responsibilities. These references to Actors are defined in [\[\_common\_data\_model\_actor\]](#_common_data_model_actor).
...
The simplest type of hard binding that can be used to detect tampering is a cryptographic hashing algorithm, as described in [Section 14.1, “Hashing”](#_hashing), over some or all of the bytes of an asset. This approach can be used on any type of asset.
...
#### 10.2.3. Asset Metadata Bindings
...
To explicitly assert the same claims in a C2PA assertion with verifiable provenance, the Exif or IPTC fields should be copied to a `stds.exif` or `stds.iptc.photo-metadata` assertion, as appropriate (see [Section 19.16, “Exif Information”](#_exif_information) and [\[\_iptc\_photo\_metadata\]](#_iptc_photo_metadata)).
...
*   The `MessageImprint` of the `TimeStampReq` structure ([RFC 3161 section 2.4.1](https://datatracker.ietf.org/doc/html/rfc3161#section-2.4.1)) shall be computed by creating the `ToBeSigned` value in [RFC 8152 section 4.4](https://datatracker.ietf.org/doc/html/rfc8152#section-4.4) with the following values for elements of `Sig_structure`:
    ...
    *   The remaining elements of `Sig_structure` are as described in [Section 14.2, “Digital Signatures”](#_digital_signatures).
...
Refer to [Section 12.3.1.4, “Embedding manifests into PDFs”](#_embedding_manifests_into_pdfs) for more information.

FLAC

Refer to [Section 12.3.1.5, “Embedding manifests into ID3”](#_embedding_manifests_into_id3) for more information.
...
Refer to [Section 12.3.1.8, “Embedding manifests into GIFs”](#_embedding_manifests_into_gifs) for more information.

DNG

Refer to [Section 12.3.1.6, “Embedding manifests into TIFF-based assets”](#_embedding_manifests_into_tiff_based_assets) for more information.
...
Refer to [Section 12.3.1.7, “Embedding manifests into RIFF-based assets”](#_embedding_manifests_into_riff_based_assets) for more information.
...
The box specified in [Section 12.3.2, “Embedding manifests into BMFF-based assets”](#_embedding_manifests_into_bmff_based_assets).
...
The portion of the manifest containing the BMFF Hash shall include the `merkle` field. Refer to [Section 10.2.2, “Hashing a BMFF-formatted asset”](#_hashing_a_bmff_formatted_asset) for more information.
...
The [CDDL Definition](https://datatracker.ietf.org/doc/html/rfc8610) for status codes is included in the schema for the ingredient assertion at [Section 19.13.5, “Schema and Example”](#ingredient_schema).
...
2.  If it is an [update manifest](#_update_manifests)
    ...
    1.  Validate that there are not any `c2pa.hash.data`, `c2pa.hash.boxes`, `c2pa.hash.bmff`, `c2pa.hash.bmff.v2`, `c2pa.actions`, `c2pa.actions.v2` or [Thumbnail](#_thumbnails) assertions. If there are, the manifest must be rejected with a failure code of `manifest.update.invalid`.
...
2.  For all other assertions:
    ...
    1.  Resolve the URI reference in the `url` field to obtain its data. If the URI does not refer to a location within the same C2PA Manifest Store (a `self#jumbf` location), the claim must be rejected. If the URI cannot be resolved and the data retrieved, the claim must be rejected with a failure code of `assertion.missing`.
        ...
        2.  Determine the hash algorithm identifier as determined by following the procedure described in [Section 14.1, “Hashing”](#_hashing):
        ...
        4.  Locate the value of the `alg` field in the allowed list or the deprecated list in [Section 14.1, “Hashing”](#_hashing) to determine the hash algorithm. If it is not present in either list, the claim must be rejected with a failure code of `algorithm.unsupported`.
...
1.  First, determine the hash algorithm to be used.
    ...
    2.  Locate the value of the `alg` field in the allowed list or the deprecated list in [Section 14.1, “Hashing”](#_hashing) to determine the hash algorithm. If it is not present in either list, the validator must abort the attempt to retrieve the external data.
...
*   If the ingredient does have a `c2pa_manifest` field:
    ...
    3.  Determine the hash algorithm identifier as determined by following the procedure described in [Section 14.1, “Hashing”](#_hashing):
...
>
> When validating a JPEG, a validator will need to check that each box identified with the special `C2PA` box identifier is indeed an `APP11` containing some or all of the C2PA Manifest Store. The C2PA Manifest Store can be identified by it being a JUMBF superbox with a label of `c2pa` and a UUID of `0x63327061-0011-0010-8000-00AA00389B71` as described in [Section 12.1.1.2, “Manifest Store”](#_manifest_store).
>
...
If the hash algorithm specified in any `alg` field does not appear in the allowed or deprecated list in [Section 14.1, “Hashing”](#_hashing), then the manifest shall be rejected with a failure code of `algorithm.unsupported`.
...
*   [Guidance for implementers](../../1.2/guidance/Guidance.html.md)
    
*   [User experience guidance](../../1.1/ux/UX_Recommendations.html.md)
...
If the value of the `type` field is `humanEntry.identified`, then an `actors` field shall contain an array of one or more actor objects as defined in [\[\_common\_data\_model\_actor\]](#_common_data_model_actor).
...
The most common way to uniquely verify the integrity of portions of a non-BMFF-based asset is via the hard bindings (i.e., cryptographic hash) present in data hash assertions. However, for those formats are "box like" but not compatible with BMFF, the [General Box Hash](#_boxes_hash) assertion is recommended.

The data hash assertion supports the creation and storage of hashes as described in [Section 14.1, “Hashing”](#_hashing), and the value shall be present in the `hash` field.
...
If the C2PA Manifest is embedded into the file, the box containing it shall be one of the entries in the exclusions array. Refer to [Section 12.3.2, “Embedding manifests into BMFF-based assets”](#_embedding_manifests_into_bmff_based_assets) for more information.
...
>
> The C2PA Manifest Store can be identified by it being a JUMBF superbox with a label of `c2pa` and a UUID of `0x63327061-0011-0010-8000-00AA00389B71` as described in [Section 12.1.1.2, “Manifest Store”](#_manifest_store).
>
> > **NOTE:**
...
When using a `c2pa.redacted` action, the `reason` field shall contain the rationale for the redaction. Additional requirements for the `c2pa.redacted` action can be found in [Section 19.12.2, “Parameters”](#_parameters).
...
For "trained algorithmic media" assets, such as those created by Generative AI, one or more [ingredients](#_ingredients) may be added to the C2PA Manifest to provide info about the inputs that led to the production of the asset. They can be referenced from a `c2pa.placed` or `c2pa.created` action as shown [below](#generative_example).
...
This assertion should be used to assert the precise location associated with the content using the Exif GPS properties (those starting `exif:GPS`). For broad information about the location, see [Section 19.17, “IPTC Photo and Video Metadata”](#_iptc_photo_and_video_metadata).
...
### 19.17. IPTC Photo and Video Metadata
...
| **11** | [https://www.iptc.org/std-dev/photometadata/specification/IPTC-PhotoMetadata-2022.1.html#alt-text-accessibility](https://www.iptc.org/std-dev/photometadata/specification/IPTC-PhotoMetadata-2022.1.html#alt-text-accessibility)\[Alt Text (Accessibility) |
