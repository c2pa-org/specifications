...
> **IMPORTANT:**
> This document is a work in progress and evolves with changes to the specification. Although, not every version of the specification will require changes to this document. In addition, each revision updates the existing guidance based on new ideas, feedback, and information.
...
The set of _C2PA Manifests_ for an _asset_ and, in the case of a _composed asset_, its _ingredients_.

> **NOTE:**
> A _C2PA Manifest_ can reference other _C2PA Manifests_.
...
[Validation](../specs/C2PA_Specification.html.md#validation-clause) of content claim signatures is critical to the C2PA security model. Validation checks provenance claim signatures and performs supporting operations. The C2PA validation model begins by confirming that a piece of content is ["Well-Formed"](../specs/C2PA_Specification.html.md#_well_formed_manifest). In short, an asset is "Well-Formed" if it can be parsed without syntax errors. A Well-Formed piece of content can also be [Valid](../specs/C2PA_Specification.html.md#_valid_manifest), if the data within the Well-Formed asset has a matching cryptographic signature that has not been revoked, and is within its validity period. A Well-Formed, Valid piece of content is considered to be [Trusted](../specs/C2PA_Specification.html.md#_trusted_manifest) if the certificate used for the valid signature chains up to a known Trust Anchor.

There are many phases of validation checks that must be completed if a piece of content is to be considered Well-Formed, Valid or Trusted. These phases include [validating the asset’s content](../specs/C2PA_Specification.html.md#_validate_the_assets_content), [validating the assertions](../specs/C2PA_Specification.html.md#_validate_the_assertions), [validating the ingredients](../specs/C2PA_Specification.html.md#_validate_the_ingredients), [validating the signature](../specs/C2PA_Specification.html.md#_validate_the_signature), [validating the time-stamp](../specs/C2PA_Specification.html.md#_validate_the_time_stamp), and [validation the credential revocation information](../specs/C2PA_Specification.html.md#_validate_the_credential_revocation_information). All of these validations must succeed in order for a piece of content to be considered Valid.

The success of the validation is returned by status codes that indicate successes, failures, and information codes as defined in [Section 15.2](../specs/C2PA_Specification.html.md#_returning_validation_results). A validator should not present C2PA manifest data to the consumer if any part of the content is not considered to be Vaild or Trusted, as defined in [section 15.3](../specs/C2PA_Specification.html.md#_displaying_manifest_information). These steps help to ensure that content consumers do not get misled by potentially maliciously altered or fake content.

The complete validation algorithm is described in detail in the [validation section of the C2PA specification](../specs/C2PA_Specification.html.md#_validation).
...
<a id="_applications_in_artifical_intelligence_ai_and_machine_learning_ml"></a>
### 2.7. Applications in Artifical Intelligence (AI) and Machine Learning (ML)

Organizations may want to leverage C2PA as a means to verify the provenance of data used to train AI and ML models. This can be one step in the overall process to help ensure that the models are trained using source data that is trustworthy. An in-depth discussion of this complex topic is provided in the C2PA [Guidance for Artificial Intelligence and Machine Learning](../../2.2/ai-ml/ai_ml.html.md).

<a id="_security_model"></a>
## 3\. Security model

The C2PA security model is focused on the confidentiality, integrity, and availability of C2PA provenance data. While attackers may aim to compromise the security of other aspects of the system, this model assumes the pivotal goal is compromise of C2PA provenance.

If a security vulnerability is ever found to be the result of a flaw in the specification, then security vulnerabilities can be reported to the C2PA via the instructions documented in the [C2PA security reporting guidelines](https://github.com/c2pa-org/specifications/SECURITY.md) on GitHub.
...
*   [Section 4.3.1, “Goal: Degrading access to provenance information”](#_goal_degrading_access_to_provenance_information)
    ...
    *   [Section 4.3.1.3, “Threat: Stripping time-stamp information”](#_threat_stripping_time_stamp_information)
        
    
*   [Section 4.3.2, “Goal: Tamper with provenance information”](#_goal_tamper_with_provenance_information)
    
    *   [Section 4.3.2.1, “Threat: Copying C2PA Manifests”](#_threat_copying_c2pa_manifests)
        
    *   [Section 4.3.2.2, “Threat: Spoofing signed C2PA Manifests via a stolen key”](#_threat_spoofing_signed_c2pa_manifests_via_a_stolen_key)
        
    *   [Section 4.3.2.3, “Threat: Spoofing signed C2PA Manifests via misuse of claim generator”](#_threat_spoofing_signed_c2pa_manifests_via_misuse_of_claim_generator)
    ...
    *   [Section 4.3.2.10, “Threat: Outdated Trust List information”](#_threat_outdated_trust_list_information)
        
    *   [Section 4.3.2.11, “Threat: Displaying revoked C2PA Manifests as valid”](#_threat_displaying_revoked_c2pa_manifests_as_valid)
        
    *   [Section 4.3.2.12, “Threat: Impersonating conforming claim generator during automated certificate enrollment”](#_threat_impersonating_conforming_claim_generator_during_automated_certificate_enrollment)
        
    *   [Section 4.3.2.13, “Threat: Tampering with assets and assertions at generation”](#_threat_tampering_with_assets_and_assertions_at_generation)
        
    *   [Section 4.3.2.14, “Threat: Downgrade attack via the specVersion field”](#_threat_downgrade_attack_via_the_specversion_field)
...
*   [Section 4.3.3, “Goal: Gain control of the C2PA implementation or environment”](#_goal_gain_control_of_the_c2pa_implementation_or_environment)
    ...
    *   [Section 4.3.3.1, “Threat: Interfering with UI rendering of C2PA Manifests”](#_threat_interfering_with_ui_rendering_of_c2pa_manifests)
    ...
    *   [Section 4.3.3.7, “Threat: Exploitation of the trustListURI”](#_threat_exploitation_of_the_trustlisturi)
        
    *   [Section 4.3.3.8, “Threat: Resource exhaustion via excessively nested ingredient manifests”](#_threat_resource_exhaustion_via_excessively_nested_ingredient_manifests)
...
*   [Section 4.3.4, “Goal: Targeting the human consumer of C2PA information”](#_goal_targeting_the_human_consumer_of_c2pa_information)
    ...
    *   [Section 4.3.4.3, “Threat: Assumed trust relationship between two identities”](#_threat_assumed_trust_relationship_between_two_identities)
        
    *   [Section 4.3.4.4, “Threat: Misuse of signed C2PA assets”](#_threat_misuse_of_signed_c2pa_assets)
        
    *   [Section 4.3.4.5, “Threat: Inability to identify artificial content”](#_threat_inability_to_identify_artificial_content)
...
*   [Section 4.3.5, “Goal: Identifying the human creator of content”](#_goal_identifying_the_human_creator_of_content)
    ...
    *   [Section 4.3.5.2, “Threat: Inability to redact title information”](#_threat_inability_to_redact_title_information)
...
<a id="_threat_stripping_time_stamp_information"></a>
##### 4.3.1.3. Threat: Stripping time-stamp information

**Description**

The C2PA specification allows for content to be cryptographically time-stamped. An attacker may obtain a copy of content, strip the original time-stamp, and replace it with a new time-stamp. By adding a time-stamp that is later than the original time-stamp, an attacker could cause confusion about when an event actually took place.

As an example: A video is recorded of a person making a significant statement that is signed and time-stamped at time T. An important event relevant to the topic of the video then occurs at time T+1. An attacker takes the original video, strips time-stamp T from the video, and then adds a new time-stamp at time T+2. This allows the attacker to claim that the person made their statement only after the event at T+1 occurred. If the fact that the statement was made before the event at T+1 occurred is critical to the understanding of the statement, then the attacker has misled people about the order of events and the context of the statement.

**Prerequisites**

An attacker needs the ability to edit C2PA content in order to remove the time-stamp. In addtion, an attacker needs access to a time-stamp server to replace the original time-stamp.

**Impact analysis**

An attacker may have the capability to mislead people as to the true order of events causing them to misinterpret the context of the content. In some situations, the order of events is critical to understanding motivations or the veracity of a statement.

**Security guidance**

To preserve the integrity of the time-stamp, consider creating a signature of the content that includes the original time-stamp. Another possible solution would be to upload the content to a verifiable location that records a copy of the content and the date it was uploaded, such as a blockchain or a social media platform.
...
<a id="_threat_copying_c2pa_manifests"></a>
##### 4.3.2.1. Threat: Copying C2PA Manifests

**Description**

An attacker copies valid, hard-bound C2PA Manifests, such as assertions, claims, or entire C2PA Manifests, from one C2PA-enabled asset to another.
...
This is similar to the case of stripping all C2PA Manifests. However, in this scenario there is an added benefit that user agents can explicitly detect the attempt to remove metadata as part of the validation process.
...
<a id="_threat_spoofing_signed_c2pa_manifests_via_a_stolen_key"></a>
...
<a id="_threat_spoofing_signed_c2pa_manifests_via_misuse_of_claim_generator"></a>
##### 4.3.2.3. Threat: Spoofing signed C2PA Manifests via misuse of claim generator
...
**Description** For some workflows, a claim generator will have assertions that it creates within its own trusted environment, referred to as "created assertions." The claim generator may also accept assertions from clients outside of its trusted environment, called "gathered assertions." Created assertions are directly attributed to the signer of the claim. Therefore, in order for the signer to maintain their reputational trust, it is important that they only include assertions in the created\_assertions field that the signer has directly created or verified. Assertion data that comes from potentially attacker controlled components should be placed in the gathered\_assertions field.

An attacker may wish to elevate the level of trust placed in assertions that they control by causing the claim generator to mistakenly place them in the created\_assertions field. Therefore, an attacker may try to cause parsing errors or other types of confusion in parsers by injecting gathered assertions that overlap with the created assertions. As an example, an untrusted client might try to send their own "claim\_generator" assertion with the intent that their gathered\_assertion value will override the created\_assertion value from the claim generator within a UI. An attacker may also try to confuse the claim generator parser by sending created assertion types within their gathered assertions in the hopes that the claim generator will inject them into the wrong field of the claim.

**Prerequisites** The attacker must be able to send customized assertions to a claim generator. The claim generator needs to have a flaw where created\_assertions and gathered\_assertions are not properly separated. Alternatively, a Content Credentials UI must have a flaw where the gathered\_assertions values override the created\_assertions values or appear to have precdent over the created\_assertions values.

**Impact analysis** An attacker may be able to override the values within the created\_assertions with their own values. This can undermine the trust that the signer associated with the claim generator has with the community. End-users viewing C2PA data may be confused as to which values are correct.

**Security guidance** Claim generators should ensure that gathered\_assertions can never be inserted into the map of created\_assertions. Claim generators may choose to have a deny list blocking claim signing requests where it is clear that an attacker is sending assertions that conflict with their created\_assertions in order to prevent abuse within their environments. C2PA UI implementations should clearly seperate gathered\_assertions from created\_assertions within the UI so that end-users can distinguish between the two sources.

<a id="_threat_outdated_trust_list_information"></a>
##### 4.3.2.10. Threat: Outdated Trust List information

**Description**

A validator relies on outdated Trust List information, from either the C2PA or from custom sources, when performing cryptographic verification of C2PA metadata. The Trust List contains critical information about trusted certificate authorities, revocation status, and certificate validity. If this information is not regularly updated, the validator may incorrectly validate manifests signed by compromised or revoked credentials, or may reject manifests signed by newly trusted authorities.

**Prerequisites**

The validator implementation lacks adequate mechanisms for detecting when Trust List data is outdated, or fails to update the Trust List on a regular basis. The attacker may exploit this by using credentials that have been revoked or compromised after the validator’s last Trust List update.

**Impact analysis**

When a validator operates with outdated Trust List information, it may present incorrect validation results to users. Manifests signed with revoked or compromised credentials may be incorrectly displayed as valid, allowing malicious content with spoofed provenance to be trusted by users. Conversely, legitimate content signed with newly trusted credentials may be incorrectly rejected. This undermines the integrity of the entire validation process and erodes user trust in C2PA technology.

The impact is particularly severe when revocation information is outdated, as attackers who have gained access to signing keys after their revocation could continue to create apparently-valid malicious content that passes validation checks.

**Security guidance**

Validator implementations must maintain and regularly update the C2PA Trust List to ensure that validation decisions are based on current, accurate trust information. The frequency of Trust List updates should be appropriate to the implementation’s use case and risk profile.

Validators should verify the integrity of Trust List data before using it for validation to ensure that validation decisions are based on authentic trust information. Implementations should include mechanisms to detect when Trust List data becomes outdated or potentially compromised, and should alert appropriate stakeholders when this occurs.

Validators must implement proper revocation checking according to C2PA specification requirements, including certificate chain validation and verification of revocation status. A clear policy for how and when revocation data will be checked should be established and communicated to users, allowing them to understand the freshness guarantees of the validation results they receive.

<a id="_threat_displaying_revoked_c2pa_manifests_as_valid"></a>
##### 4.3.2.11. Threat: Displaying revoked C2PA Manifests as valid

**Description**

An attacker gains access to a claim signing key that has been revoked, or creates C2PA metadata before a key is revoked and distributes it afterward. A validator that does not properly check revocation status displays this revoked metadata as valid to users, leading them to trust content with compromised provenance.

**Prerequisites**

The attacker needs access to a signing key that has been or will be revoked. The target validator implementation must have inadequate revocation checking mechanisms, or must be operating with outdated revocation information. Users must view the C2PA metadata through the vulnerable validator.

**Impact analysis**

When validators display revoked C2PA metadata as valid, users may trust content that should be considered compromised. This could occur when a signing key is stolen and later revoked, when a certificate authority is compromised, or when credentials are revoked for policy violations. The time window between when a key is compromised and when it is revoked creates an opportunity for attackers to create malicious content that some validators may continue to display as valid indefinitely if they do not properly check revocation status.

This threat undermines the certificate revocation mechanism, which is a critical security control for responding to key compromise and other trust failures. Users who rely on validation results from implementations with poor revocation checking may be exposed to content with spoofed or misleading provenance.

**Security guidance**

Validator implementations must implement robust revocation checking as part of their cryptographic validation process. This includes verifying the revocation status of all certificates in the certificate chain according to C2PA specification requirements.

Validators should establish and document clear policies for how and when revocation checking will be performed. These policies should be transparent to users so they understand the level of assurance provided by the validation results. Implementations should consider the trade-offs between security, performance, and availability when designing their revocation checking strategy.

For implementations that cache or staple revocation responses (such as OCSP responses), the validity period of cached revocation information should be carefully chosen to balance performance with security requirements. Validators should handle cases where revocation information is unavailable in a secure manner, with clear communication to users about any limitations in the validation results.

Validators should maintain fresh Trust List information to ensure they have current revocation data. When revocation checks fail or cannot be completed, the validator should communicate this limitation clearly in the validation results presented to users.

Please refer to the [C2PA specification](../specs/C2PA_Specification.html.md#_revocation_checking) for more details on revocation checking.

<a id="_threat_impersonating_conforming_claim_generator_during_automated_certificate_enrollment"></a>
##### 4.3.2.12. Threat: Impersonating conforming claim generator during automated certificate enrollment

**Description**

An attacker attempts to impersonate a conforming claim generator instance during an automated certificate enrollment process with a Certification Authority. Automated certificate enrollment flows allow conforming claim generator instances to enroll for certificates or rotate certificates on an ongoing basis without human intervention. An attacker may exploit weaknesses in the automated enrollment authentication process to obtain a valid claim signing certificate for an attacker-owned key pair.

**Prerequisites**

Ability to access and interact with the Certification Authority’s automated certificate enrollment process. Knowledge of or ability to bypass the authentication methods used by the CA to verify the identity and eligibility of claim generator instances requesting certificates.

**Impact analysis**

If successful, an attacker could obtain legitimate claim signing certificates without being a valid conforming claim generator. These certificates would be trusted by validators, allowing the attacker to create C2PA Manifests that pass validation checks and appear to come from a legitimate, conforming generator. The attack would be limited to the specific CA’s trust domain, but could enable the attacker to create misleading provenance that would be trusted by systems and users relying on that CA’s certificates.

**Security guidance**

Claim generators using automated certificate enrollment should implement secure authentication methods as required by their Certification Authority. These may include shared secrets, client certificates, username/password combinations, challenge-response mechanisms, or symmetric key MAC. Authentication secrets must be properly managed and protected, particularly for edge implementations where secrets should not be embedded in binaries.

For enhanced security, claim generator implementations should be capable of producing verifiable artifacts backed by a hardware Root of Trust, such as attestations or hardware-derived credentials from the underlying platform. These artifacts can confirm the identity of the claim generator binary through package names, hashes, code signing certificates, or other digital certificates. Certification Authorities should validate these hardware-backed attestations during the enrollment process to ensure that only legitimate claim generator instances can obtain certificates.

<a id="_threat_tampering_with_assets_and_assertions_at_generation"></a>
##### 4.3.2.13. Threat: Tampering with assets and assertions at generation

**Description**

An attacker compromises software components within the claim generator that process or modify digital content and assertions during the generation process. By exploiting vulnerabilities in these components, an attacker could tamper with the asset or assertions before they are signed, resulting in misleading provenance data being cryptographically bound to modified content.

**Prerequisites**

Ability to exploit vulnerabilities in software components of the claim generator that handle asset processing or assertion generation. This may include vulnerabilities in the main claim generator code, third-party libraries, or supporting software components. The attacker needs the ability to inject, modify, or substitute data during the generation process before the claim is signed.

**Impact analysis**

Successful exploitation could allow an attacker to manipulate digital content or assertions during the generation process while still producing validly-signed C2PA Manifests. Since the tampering occurs before signing, the resulting manifests would pass validation checks but would contain incorrect or misleading provenance information. This undermines the integrity of the provenance chain, as the signed metadata would not accurately represent the actual processing that occurred.

The scope of impact depends on which software components are compromised. Exploitation of widely-used third-party libraries could affect multiple claim generator implementations. Compromise of specific claim generator implementations would be limited to content produced by those generators.

**Security guidance**

Claim generators should implement comprehensive security practices for all software components that process digital content and assertions. This includes performing regular third-party vulnerability analysis to detect vulnerabilities from sources such as the NIST National Vulnerability Database. Implementations should enable basic exploit countermeasures such as secure compiler flags. Static analysis should be performed on claim generator code and associated components. Access control and binary authentication should be enforced for claim generator binaries, preferably by an environment with higher privilege levels than the claim generator itself.

For distributed and backend implementations, the claim generator should be capable of producing verifiable artifacts backed by a hardware Root of Trust that confirm the integrity and patch status of all software components involved in asset and assertion processing. Process isolation and protection of inter-process communication channels should be implemented using kernel or operating system level protections to prevent tampering during the generation workflow.

<a id="_threat_downgrade_attack_via_the_specversion_field"></a>
##### 4.3.2.14. Threat: Downgrade attack via the specVersion field

**Description**

An attacker exploits the C2PA Claim `specVersion` field to try to cause software processing the C2PA claim to apply more lenient validation or parsing rules associated with older versions of the specification. The C2PA specification includes a `specVersion` field that informs the consumer of the claim which version of the specification was used to create the claim. If vulnerabilities or weaker validation requirements exist in prior specification versions, an attacker may set this field to an older version number to attempt to trigger less stringent validation or parsing, allowing exploits that would otherwise be prevented by current specification requirements.

**Prerequisites**

The attacker needs access to a claim generator that allows the specification of the `specVersion` field. The claim generator must either be based on a version of the specification with known weaknesses, or must allow the attacker to arbitrarily set the `specVersion` field. The target system must incorrectly honor the `specVersion` field by applying validation logic corresponding to the specified version, rather than enforcing current specification requirements.

**Impact analysis**

An attacker could exploit validation weaknesses present in earlier versions of the C2PA specification after those issues have been addressed in newer versions. This effectively allows attackers to bypass security improvements and exploit known vulnerabilities indefinitely, undermining the security evolution of the specification.

The severity of impact depends on the nature of vulnerabilities present in earlier specification versions. Potential consequences could include bypassing cryptographic validation checks, evading revocation checking, or exploiting parsing vulnerabilities that have been addressed in later versions. This type of downgrade attack could allow creation of malicious content that appears valid to software honoring older specification versions.

**Security guidance**

The claim `specVersion` field is an optional, informational field to be used only for determining whether the receiving system supports the stated version of the specification.

Implementations should always apply the latest specification version for all security-relevant validation, including cryptographic signature verification, certificate chain validation, revocation checking, and timestamp verification. This ensures that security improvements are consistently applied regardless of the value of the `specVersion` field.

Implementations should be designed with clear policies on version handling and should document which aspects of validation, if any, are affected by the `specVersion` field. In addition, implementations should denote in their results what version of the specification was used for validation so that the consumer is aware.
...
<a id="_threat_interfering_with_ui_rendering_of_c2pa_manifests"></a>
##### 4.3.3.1. Threat: Interfering with UI rendering of C2PA Manifests
...
The viewer of the claim metadata will be misled with regards to the validity of the image. As an example, if the malicious payload contained attacker controlled JavaScript that was interpreted by the web page displaying the Content Credential to the end-user, then the attacker could manipulate the UI so that the viewer would see fake signing information or incomplete claim information.
...
It is important to specify the `version` within the `generator-info-map` defined by the C2PA specification and to increment it with new releases. If the claim generator software or service ever has a vulnerability that allows an attacker to manipulate important assertions or fields, then that will cause doubt around content created with that release. Content consumers will want to know whether a piece of content was created before or after that issue was patched. By having a strong versioning strategy, content consumers can confirm that the data is reliable by verifying that it was created with a patched version.
...
<a id="_threat_exploitation_of_the_trustlisturi"></a>
##### 4.3.3.7. Threat: Exploitation of the trustListURI

**Description**

The C2PA specification introduced the _trustListURI_ field in version 2.3 of the specification. This field is a URI that identifies the trust list that was used to validate the claim signature. An attacker may try both parsing attacks as well as social engineering attacks to manipulate a parser or renderer of the _trustListURI_.

**Prerequisites**

The attacker needs to have the ability to generate content with a valid claim signature that is signed by a trusted certificate authority. The attacker also needs to have the ability to manipulate the _trustListURI_ field.

**Impact analysis**

The _trustListURI_ is a Universal Resource Identifier (URI). As such, this field is subject to the same parsing and validation attacks as any other URI. Since URLs are a subset of URIs, the _trustListURI_ would be subject to the same attacks as any other URL, such as the use of malicious schemes or references to malicious sites. As an attacker controlled value, the _trustListURI_ could be used to confuse or crash the parser used to validate the trust list URI. It is important to note that a _trustListURI_ only needs to be a unique identifier and it is not required to be a valid URL. Unsuccessfully parsing a malicious value could result in arbitrary code execution or other malicious behavior. If the value is displayed to a human, then the value could also be used to socially engineer the user. This might include typosquatting or homoglyph attacks intended to confuse the user into trusting a malicious entity. In addition, if parsers attempt to automatically fetch alternative trust lists based on the URI, then those network requests could be used to track users who are viewing the content. It is recommended that the _trustListURI_ only be a reference to a known trust list rather than the actual URL.

**Security guidance**

The _trustListURI_ field is informative. If the C2PA Trust List was used to validate the claim signature, the _trustListURI_ field shall not be present. Parsers that consume the field need to be robust against potential malicious values. If URLs are accepted, then the parser should validate that they conformed to allowed schemes and potentially limit URLs to sites on an allow list of supported lists. If this field is displayed to a human, then it should do so with caution as the value could be part of a social engineering attack or a pointer to a malicious site. The display should not imply or indicate that the data validated by a trust list other than the C2PA Trust List is at the same level of trust as the C2PA Trust List. If the parser uses the _trustListURI_ value to fetch an alternative trust list, then the software should make the network request using a method that protects the privacy of the user.

<a id="_threat_resource_exhaustion_via_excessively_nested_ingredient_manifests"></a>
##### 4.3.3.8. Threat: Resource exhaustion via excessively nested ingredient manifests

**Description**

An attacker creates C2PA content with an extremely long chain of recursively nested ingredient manifests. C2PA Manifests can reference ingredient manifests, which are nested pieces of content that describe the provenance history of assets used in creating the current asset. By creating content with excessive nesting depth or an extremely large number of ingredients at each level, an attacker can attempt to exhaust the memory, processing capacity, or rendering capabilities of a validator or user interface attempting to process or display the manifest hierarchy.

**Prerequisites**

The attacker needs the ability to create C2PA content with arbitrarily deep ingredient nesting or a large number of ingredients. This could be accomplished through access to a claim generator that allows specifying ingredients, or by crafting malicious C2PA manifests directly. The target validator or user interface must lack adequate protections against processing deeply nested or excessively large manifest structures.

**Impact analysis**

Successful exploitation could lead to several adverse outcomes. The validator application may exhaust its available memory while attempting to parse or process the entire chain of nested ingredients, potentially causing the application to crash or become unresponsive. This represents a denial-of-service attack against the validator.

Even if memory exhaustion does not occur, excessive processing time required to traverse deeply nested ingredient chains could make the validator unusable or significantly degrade performance. For validators with time limits on processing, the content may time out and fail validation, even though the cryptographic signatures may be valid.

User interfaces attempting to display the complete provenance history may also be overwhelmed. If the UI attempts to enumerate and render each level of nested content in a single view, it could become unresponsive, display incorrectly, or crash. Even if the UI remains functional, users may be unable to comprehend or navigate an excessively complex ingredient hierarchy, potentially leading to confusion about the actual provenance of the content.

In some cases, the resource exhaustion could be severe enough to impact other applications or users on a shared system, extending the denial-of-service beyond just the validator application itself.

**Security guidance**

Validator implementations should implement resource management controls to protect against excessively nested or large ingredient manifest structures. This includes setting reasonable limits on the maximum depth of ingredient nesting that will be processed, and limits on the total number of ingredients that will be examined. If the limit is hit, then the user should be informed that the entire file could not be processed and given appropriate options for handling the situation.

When processing ingredient manifests, validators should use iterative rather than recursive algorithms where possible to avoid stack overflow issues with deeply nested structures. Memory usage should be carefully managed, potentially processing ingredients in batches or on-demand rather than loading the entire ingredient hierarchy into memory at once.

Implementations should validate the structure of the manifest before attempting full processing, checking for excessive nesting depth or ingredient counts and failing gracefully if thresholds are exceeded. Clear error messages should be provided to users when content is rejected due to excessive complexity.

User interfaces displaying provenance information should use paginated or on-demand rendering techniques rather than attempting to display the entire ingredient hierarchy at once. Implementations can provide summary views showing key information about the ingredient chain (such as the total depth or number of ingredients) with options for users to drill down into specific levels or branches of interest.

User interfaces should provide clear visual indicators of the depth and complexity of ingredient chains, helping users understand when content has an unusually complex provenance history. For excessively nested content, UIs should clearly communicate that the full provenance cannot be displayed in a single view and provide navigation tools for exploring the structure.

Validators should monitor resource consumption during manifest processing and implement timeout mechanisms to prevent indefinite processing of malicious content. If resource limits or timeouts are reached, the validator should fail securely with appropriate error handling and user notification.

Regular testing with complex files, including excessively nested ingredient structures, should be performed to validate that implementations handle these scenarios gracefully without crashes or excessive resource consumption.
...
<a id="_threat_assumed_trust_relationship_between_two_identities"></a>
##### 4.3.4.3. Threat: Assumed trust relationship between two identities
...
The attacker needs to have a valid certificate that is included in a trust list used by the platform presenting the Content Credentials.
...
<a id="_threat_inability_to_identify_artificial_content"></a>
##### 4.3.4.5. Threat: Inability to identify artificial content

**Description**

It is reasonable to expect that content consumers will want to be able to identify artificial content. However, there are many different types of artificial content that can be created. For instance, an image may be a slight modification of a trusted image or it could be a complete fabrication generated by AI/ML. If artificially generated or modified images are not consitently labeled to note the inclusion of artificial content, then the content consumer may lose trust in the source and the claim generator.

**Prerequisites**

The attacker needs to have the ability to leverage AI/ML tools to create or modify content without the tool recording the use of AI/ML in the resulting signed C2PA manifest. This could also be the result of an implementation that does not record the use of AI/ML in the resulting signed C2PA manifest in accordance with best practices.

**Impact analysis**

Content consumers may lose trust in the source of the content or the claim generator if they are not able to identify artificial content or feel that the content is not providing the information they expect.

**Security guidance**

The C2PA specification supports providing information regarding how AI/ML was used to create or modify content in several ways. The Asset Type assertion can be used to specify the type of the model and dataset assets. In addition, digitalSourceType field for the c2pa.created action can be specified with the value: `[http://cv.iptc.org/newscodes/digitalsourcetype/trainedAlgorithmicMedia](http://cv.iptc.org/newscodes/digitalsourcetype/trainedAlgorithmicMedia)` to indicate the use of AI/ML to create the content. A collection data hash assertion can be used to describe each folder of a training data set for an AI/ML model. The Ingredient Relationship inputTo field can be used to indicated that this ingredient was used as input to an AI/ML model that led to the creation or modification of this asset. Refer to the C2PA specification for complete details on the available assertions for providing information about the use of AI/ML to create or modify the content.

In addition, refer to the C2PA [User Experience Guidance](../../2.2/ux/UX_Recommendations.html.md) for details on how to display this information to an end-user.
...
The privacy and anonymity of the consumer is compromised. In some contexts, this could endanger the consumer.
...
<a id="_threat_inability_to_redact_title_information"></a>
##### 4.3.5.2. Threat: Inability to redact title information

**Description**

The C2PA specification allows for placing information in the `dc:title` field of claims and ingredient maps. The information placed in those locations can not be redacted because it is not an assertion. Starting in version 2.4, the specification recommends that any title information that might be sensitive to be placed in a metadata assertion. This will ensure that the information can be redacted in the future if necessary.

**Prerequisites**

The dc:title information in the claim and ingredient maps are readable by anyone and can not be redacted by later versions. If a claim generator places identifying information into that field, the attacker only has to read that field.

**Impact analysis**

The privacy and/or anonymity of the author may be compromised. In some context, this could endanger the author.

**Security guidance**

Ensure that any sensitive dc:title information is stored in a metadata assertion rather than within a claim or ingredient map.
...
6.  **Pre-existing information stores** Any revocation plan should take into account places where the image or the Content Credentials may be stored or cached. For instance, Content Delivery Networks (CDNs) may have caches of the images that persist and need to be cleared. In addition, an image’s watermark or digital fingerprint may have been stored by online services along with the original Content Credential. When a search is conducted via these online services, the original Content Credential with the revoked certificate may be returned. Planning for a revocation includes planning for how to handle situations when customers encounter older versions of the image based on the revoked certificate.
