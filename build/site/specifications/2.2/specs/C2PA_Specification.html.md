...
A representation of an [asset](#_asset) (either as a part of an [asset](#_asset) or a completely new [asset](#_asset)) where the [digital content](#_digital_content) has had a ['non-editorial transformation'](#_editorial_transformation) [action](#_action) (e.g., re-encoding or scaling) applied.
...
The digital signature on the [claim](#_claim) created using the private key owned by a [signer](#signer-definition). The [claim signature](#_claim_signature) is a part of the [C2PA Manifest](#_c2pa_manifest).
...
*   [X.509 Certificate General-Purpose Extended Key Usage (EKU) for Document Signing](https://datatracker.ietf.org/doc/html/rfc9336)
...
*   Changes and improvements in various hard binding assertions
    ...
    *   Add support for use of a `c2pa.hash.bmff` assertion in an update manifest
...
Some assertion data, due to its size or an infrequent need for it, may be externally hosted. Such data are not embedded in the assertion store, but instead are referenced by URI. This is accomplished through a cloud data assertion (see [Section 18.11, “Cloud Data”](#_cloud_data)). Unlike embedded assertion data, cloud data is not retrieved nor validated as part of manifest validation, and are only retrieved and validated when specifically needed by an application according to a different set of validation rules as described in [Section 15.10, “Validate the Assertions”](#_validate_the_assertions).
...
The default specification for a date and/or time value in an assertion is the date/time format and serialized in CBOR as tag number `0` ([RFC 8949](https://datatracker.ietf.org/doc/html/rfc8949), 3.4.1) and represented in CDDL as type `tdate`.
...
#### 9.3.2. List of Allowed Soft Binding Algorithms
...
###### 15.10.3.2.4. `c2pa.time-stamp` validation
...
If the hash computed over all the asset’s data (minus any exclusion ranges) does not match the value of the `hash` field in the `c2pa.hash.data`, then the validator shall look for presence of a [multi-asset hash assertion](#_multi_asset_hash). If one is present, it shall be validated as described in [Section 15.12.4, “Validating a multi-asset hash”](#_validating_a_multi_asset_hash), but if one is not present, the manifest shall be rejected with a failure code of `assertion.dataHash.mismatch`.
...
If the `hash` field is not present, or any resultant hash does not match the value of the `hash` field for those boxes, then the manifest shall be rejected with a failure code of `assertion.boxesHash.mismatch`. If the box hashing process produces a `assertion.boxesHash.mismatch` failure code, then the validator shall look for presence of a [multi-asset hash assertion](#_multi_asset_hash). If one is present, it shall be validated as described in [Section 15.12.4, “Validating a multi-asset hash”](#_validating_a_multi_asset_hash), but if one is not present, the manifest shall be rejected with a failure code of `assertion.boxesHash.mismatch`.
...
An actions assertion provides information on edits and other actions taken that affect the asset’s content. There will be an array of actions - each action declaring _what_ took place on the asset and (optionally) _when_ it took place, along with possible other information such as what software performed the action. Except where noted in [Section 18.14.2, “Mandatory presence of at least one actions assertion”](#_mandatory_presence_of_at_least_one_actions_assertion), the order of actions in this array is unspecified, and does not imply the order in which actions were performed.
...
#### 18.14.2. Mandatory presence of at least one actions assertion
...
#### 18.14.4. Fields in the actions assertion
...
When using a `c2pa.redacted` action, the `reason` field shall contain the rationale for the redaction. Additional requirements for the `c2pa.redacted` action can be found in [Section 18.14.4.7, “Parameters”](#_parameters).
...
#### 18.15.2. Establishing unique identifiers
...
When adding an ingredient assertion, a claim generator shall add a `c2pa.actions` assertion (see [Section 18.14, “Actions”](#_actions)), if one does not already exist in the active manifest. Depending on the type of ingredient, one of the following new entries shall be added to the `actions` array of a `c2pa.actions` assertion.
...
#### 18.15.8. Ingredient Data

<a id="_standard_usage"></a>
##### 18.15.8.1. Standard Usage
...
##### 18.15.8.2. Multi-file Ingredients
...
#### 18.15.12. Copying existing manifests

<a id="_determining_the_need"></a>
##### 18.15.12.1. Determining the need
...
1.  Validate the ingredient per the process described in [Section 18.15.12.4, “Ingredient validation”](#_ingredient_validation). In case of validation failures, the claim generator may skip the rest of these steps if directed to do so (for example, via user input or via configuration).
...
##### 18.15.12.3. Adding manifest references to the ingredient assertion
...
##### 18.15.12.4. Ingredient validation

<a id="_general_18"></a>
###### 18.15.12.4.1. General
...
###### 18.15.12.4.2. V2 ingredient assertions (DEPRECATED)
...
###### 18.15.12.4.3. V3 ingredient assertions
...
#### 18.16.3. The `c2pa.metadata` assertion
...
#### 18.16.4. Redaction of `c2pa.metadata`
...
#### 18.20.3. Details on selection of a value for `type`
...
Refer to [Section A.3.8, “Embedding manifests into JPEG XL”](#_embedding_manifests_into_jpeg_xl) for more information.
...
Refer to [Section A.3.7, “Embedding manifests into GIFs”](#_embedding_manifests_into_gifs) for more information.

DNG

Refer to [Section A.3.5, “Embedding manifests into TIFF-based assets”](#_embedding_manifests_into_tiff_based_assets) for more information.
...
Refer to [Section A.3.6, “Embedding manifests into RIFF-based assets”](#_embedding_manifests_into_riff_based_assets) for more information.
...
Refer to [Section A.3.9, “Embedding manifests into fonts”](#_embedding_manifests_into_fonts) for more information.
...
Figure 19. A C2PA Manifest embedded in a JPEG XL image
...
Figure 20. Example of a PDF with multiple ingredient manifests
