...
*   \[ISO/IEC 18181-2:2021\], _Information technology — JPEG XL image coding system — Part 2: File format_
    
*   \[ISO/IEC 18477-1:2020\], _Information technology — Scalable compression and coding of continuous-tone still images — Part 1: Core coding system specification_
...
*   \[IPTC Photo Metadata Standard\], _IPTC Photo Metadata Standard 2023.1_, [https://www.iptc.org/std/photometadata/specification/IPTC-PhotoMetadata](https://www.iptc.org/std/photometadata/specification/IPTC-PhotoMetadata)
...
collection of information that can inform a {{manifest consumer}}'s judgment of the trustworthiness of an {{asset}}. These are in addition to the {{signer}} of a {{claim}}, upon which the fundamental trust model relies
...
> **NOTE:**
> Previous versions of this specification used [UUID](#RFC9562) URN, and had the identifier of the claim generator at the beginning of the URN. However, this was found to be not in compliance with either [UUID](#RFC9562) or [URN](#RFC8141).
...
All references to information in the manifest, whether stored internally to the asset (i.e., embedded) or stored externally to the asset (e.g., in the cloud), shall be referenced via JUMBF URI references as defined in [\[ISO/IEC 19566-5:2023\]](#ISO19566-5), C.2. These URIs shall be used either as part of a `hashed_uri` or `hashed_ext_uri` data structure.
...
#### 4.3.1. List of Allowed Soft Binding Algorithms
...
If possible, the signer should use a RFC 3161-compliant Time Stamp Authority (TSA) ([\[RFC 3161\]](#RFC3161)) to obtain a trusted time-stamp proving that the signature itself actually existed at a certain date and time and incorporate that into the `COSE_Sign1_Tagged` structure as a countersignature.

A time-stamp manifest shall never follow this procedure, as the signer of the manifest itself is a TSA.

Signers are encouraged to obtain and include time-stamps to ensure their manifests will remain valid. As described in [Validation](#validation-clause), manifests without time-stamps cease to be valid when the signing credential expires or becomes revoked. A manifest shall contain only one time-stamp.
...
A previous version of this specification used the same value for the `payload` field in the time-stamp as was used in the `Sig_signature` as described in [Signing a Claim](#_signing_a_claim). This payload is henceforth referred to as a "v1 payload" in a "v1 time-stamp" and is considered deprecated. A claim generator shall not create one, but a validator shall process one if present.
...
v2 time-stamps shall be stored in a COSE unprotected header whose label is the string `sigTst2`. When present, the value of this header shall be a `tstContainer` defined by [CDDL for `tstContainer`](#tstContainer-CDDL). The content of value of the `timeStampToken` field of the `TimeStampResp` structure received in reply from the TSA shall be stored as the value of the `val` property of an element of `tstTokens`.
...
##### 6.1.3.1. Compressed boxes

In order to support compressing manifests, a new `brob` content box is supported by C2PA. Based on a similar box in JPEG-XL ([\[ISO/IEC 18181-2:2021\]](#ISO18181-2)), the `brob` box is a content box whose contents are the [Brotli-compressed](https://datatracker.ietf.org/doc/html/rfc7932) bytes of either a [standard manifest](#_standard_manifests) or [update manifest](#_update_manifests), as described in the [compressed manifests](#_compressed_manifests) clause. The `brob` box shall have box ID of `0x62726F62` (`brob`).
...
#### 8.2.5. Cryptographic validation
...
#### 8.2.6. Inclusion of Signer Icons
...
X.509 certificates support revocation status queries. A claim generator should use the Online Certificate Status Protocol (OCSP, see [\[RFC 6960\]](#RFC6960)) and OCSP stapling (as originally conceptualized in [clause=8](#RFC6066), but implemented as described in this clause) to implement revocation. The claim generator shall not use Certificate Revocation Lists (CRLs, see [\[RFC 5280\]](#RFC5280)). \`\`
...
A conforming CA should include an AuthorityInfoAccess (AIA) extension ([\[RFC 5280\]](#RFC5280), section 4.2.2.1) to provide access information for the OCSP service operated by the CA.
...
#### 10.2.1. Standard Status Codes

<a id="_success_codes"></a>
##### 10.2.1.1. Success codes
...
If the C2PA Manifest is a [time-stamp manifest](#time_stamp_manifests), the signature shall be validated as described in [Time Stamp Authorities](#_time_stamp_authorities). For all other types of C2PA Manifests, the validation of the credential used in the signature shall be performed according to [Trust Model](#_trust_model).
...
#### 10.9.1. Determining revocation through OCSP response in `rVals` header

A validator shall decode the OCSP response per the requirements of [\[RFC 6960\]](#RFC6960), in particular [\[RFC 6960\]](#RFC6960), section 3. If it is accepted and if all of the following requirements are met, then this establishes that the signer’s certificate was not revoked at the time of signing.
...
*   The signer of the response is an "authorized responder" as defined by [\[RFC 6960\]](#RFC6960), section 4.2.2.2.
...
#### 10.9.2. Determining revocation for online OCSP response

If the `rVals` header is not present, or if the manifest does not have a time-stamp, the validator may choose to query the OCSP responder per the information in the AuthorityInfoAccess (AIA) extension ([\[RFC 5280\]](#RFC5280), section 4.2.2.1) of the [X.509 Certificates](#x509_certificates).
...
If a response is received and accepted per the requirements of [\[RFC 6960\]](#RFC6960), it shall establish the signer’s certificate was not revoked at the time of signing if either of the following requirements is fulfilled:
...
##### 10.10.1.4. Time-Stamp Manifest Assertions
...
For more details, please see the [Performing explicit validation](#_performing_explicit_validation) section.
...
*   `c2pa.ingredient` or `c2pa.ingredient.v2` or `c2pa.ingredient.v3`, [Validate the Ingredients](#_validate_the_ingredients)
    
*   `c2pa.metadata`, [`c2pa.metadata` validation](#_c2pa_metadata_validation)
    

If the value of any field of an assertion is of type `hashed_uri` or `hashed_ext_uri`, the validator shall perform the steps described in [Validation of References](#_validation_of_references).
...
If a `hashed_ext_uri` is present, the validator shall perform the steps described in [Validation of External References](#_validation_of_external_references). If a `hashed_uri` is present, the validator shall perform the steps described below.
...
6.  Validate all assertions in the claim, as described in [Validate the Assertions](#_validate_the_assertions).
...
If the `merkle` field in the `bmff-hash-map` is present, the validator must validate the Merkle tree. If the auxiliary `uuid` C2PA box doesn’t exist as described in [Auxiliary `'c2pa'` Boxes for Large and Fragmented Files](#_auxiliary_c2pa_boxes_for_large_and_fragmented_files), then the manifest shall be rejected with a failure code of `assertion.bmffHash.malformed`. If the hash calculated from the auxiliary `uuid` C2PA box and leaf node doesn’t match the element of `hashes` in the `bmff-merkle-map` is not equal, then the manifest shall be rejected with a failure code of `assertion.bmffHash.mismatch`

<a id="_validating_a_general_boxes_hash"></a>
#### 10.12.3. Validating a general boxes hash
...
When validating a JPEG, a validator will need to check that each box identified with the special `C2PA` box identifier is indeed an `APP11` containing some or all of the C2PA Manifest Store. The C2PA Manifest Store is identified by it being a JUMBF superbox with a label of `c2pa` and a JUMBF type UUID of `0x63327061-0011-0010-8000-00AA00389B71` as described in [Manifest Store](#_manifest_store).
...
If a `dateTime` field is present, its value shall be a date time string that complies with [\[ISO 8601-1:2019\]](#ISO8601).
...
A previous version of this specification provided a `url` field to provide a pointer to where the hashed data can be located, but it was never used. This field is now deprecated in favour of the [asset reference assertion](#_asset_reference). Claim generators shall not add this field to a data hash assertion, and consumers shall ignore the field when present, except this shall not affect inclusion of the field as part of the content being validated as described in [Assertion Validation](#_assertion_validation).
...
A previous version of this specification provided a `url` field to provide a pointer to where the hashed data can be located, but it was never used. This field is now deprecated in favour of the [asset reference assertion](#_asset_reference). Claim generators shall not add this field to a BMFF hash assertion, and consumers shall ignore the field when present, except this shall not affect inclusion of the field as part of the content being validated as described in [Assertion Validation](#_assertion_validation).
...
Figure 14. Validating the initialization segment

![Validating the chunk’s data](_images/BMFF_Validation-Chunks.svg)

Figure 15. Validating the chunk’s data

<a id="_general_boxes_hash"></a>
### 11.7. General Boxes Hash
...
#### 11.7.2. Handling for specific formats

<a id="_jpeg_specific_handling"></a>
##### 11.7.2.1. JPEG-specific Handling
...
The C2PA Manifest Store can be identified by it being a JUMBF superbox with a label of `c2pa` and a JUMBF type UUID of `0x63327061-0011-0010-8000-00AA00389B71` as described in [Manifest Store](#_manifest_store).
...
The [\[MPF\]](#MPF) extension to JPEG can also be supported using this method by listing all boxes contained in the file as they appear.

<a id="_png_specific_handling"></a>
##### 11.7.2.2. PNG-specific Handling
...
##### 11.7.2.3. TIFF-specific Handling
...
##### 11.7.2.4. GIF-specific Handling
...
##### 11.7.2.5. RIFF-specific Handling
...
##### 11.7.2.6. Font-specific Handling
...
#### 11.7.3. Schema and Example

The schema for this type is defined by the `box-map` rule in the [CDDL Definition](#RFC8610) in [CDDL for Boxes Hash](#cddl_for_boxes_hash):
...
#### 11.9.4. Soft Binding Algorithm List
...
#### 11.12.2. Mandatory presence of at least one actions assertion
...
#### 11.12.3. Fields in the actions assertion
...
Also present may be the date and time when the action took place in the `when` field. If included, the value of the `when` field shall be compliant with [\[ISO 8601-1:2019\]](#ISO8601).
...
#### 11.12.6. Action Templates

<a id="_common_elements"></a>
##### 11.12.6.1. Common Elements

<a id="_templates"></a>
###### 11.12.6.1.1. Templates
...
#### 11.13.2. Establishing unique identifiers

If the ingredient being added contains a C2PA Manifest, then its unique identifier shall be taken from the `urn:c2pa` value of the JUMBF superbox containing the ingredient’s active C2PA Manifest, and it is not necessary to provide the optional `instanceID` field of the ingredient assertion. When the claim generator provides the optional `instanceID` field of the ingredient assertion, then the value of the unique identifier shall be determined as specified by [Identifying Non-C2PA Assets](#_identifying_non_c2pa_assets).
...
#### 11.13.8. Ingredient Data

<a id="_standard_usage"></a>
##### 11.13.8.1. Standard Usage
...
##### 11.13.8.2. Multi-file Ingredients
...
###### 11.13.12.1.3. V3 ingredient assertions
...
#### 11.14.3. The `c2pa.metadata` assertion
...
#### 11.14.4. Redaction of `c2pa.metadata`
...
#### 11.16.3. Details on selection of a value for `type`
...
Figure 17. A C2PA Manifest embedded in a JPEG XL image
...
Figure 18. Example of a PDF with multiple ingredient manifests
